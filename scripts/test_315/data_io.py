...
def checkargs(args):
    if args.ligand not in ['DNA', 'RNA', 'PIS']:
...
if __name__ == '__main__':
    ...
    # testset_dict = {'PPIS': 'Test_315.txt'}
    testset_dict = {'PPIS': 'SJT1.txt'}
    ...
    Dataset_dir_315 = Dataset_dir + '/dataset_dir_SJT'
    #数据集id合集
    seqlist = test_list
    ...
    # psepos:核心原子
    cal_Psepos(seqlist, PDB_DF_dir, Dataset_dir_315, psepos, ligand, seqanno)
    ...
    cal_PSSM(ligand, seqlist, Dataset_dir + '/PSSM', Dataset_dir_315)
    #cal_HMM(ligand, seqlist, Dataset_dir + '/HMM', Dataset_dir)
    cal_DSSP(ligand, seqlist, Dataset_dir + '/SS', Dataset_dir_315)
    #处理原子特征
    PDBResidueFeature(seqlist, PDB_DF_dir, Dataset_dir_315, ligand, feature_list, feature_combine, atomfea)

    root_dir = Dataset_dir_315 + '/' + ligand + '_{}_dist{}_{}'.format(psepos, dist, feature_combine)
    raw_dir = root_dir + '/raw'
    if os.path.exists(raw_dir):
        shutil.rmtree(root_dir)
    os.makedirs(raw_dir)
    print('4.Calculate the neighborhood of residues. Save to {}.'.format(root_dir))
    Create_NeighResidue3DPoint(psepos, dist, Dataset_dir_315, raw_dir, seqanno, feature_combine,
                               test_list)
