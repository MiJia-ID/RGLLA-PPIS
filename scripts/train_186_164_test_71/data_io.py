...
def parse_args():
    ...
    parser.add_argument("--ligand", dest="ligand", default='PIS',
...
def checkargs(args):
    if args.ligand not in ['PIS','DNA', 'RNA']:
...
def cal_DSSP(ligand, seq_list, dssp_dir, feature_dir):
    ...
    for seqid in seq_list:
        file = f"{seqid}.dssp"
        dssp_file = os.path.join(dssp_dir, file)

        try:
            with open(dssp_file, 'r') as fin:
                fin_data = fin.readlines()
        except FileNotFoundError:
            print(f"文件 {dssp_file} 未找到，跳过")
            continue
        ...
        # 解析 DSSP 数据
        for i in range(start_index, len(fin_data)):
            line = fin_data[i].rstrip("\n")  # 去除换行符
            line_length = len(line)

            try:
                # 提取数据，动态检查字段是否存在
                aa = line[13] if line_length > 13 else ' '
                if aa not in maxASA:
                    continue

                res_id = safe_float(line[5:10]) if line_length > 10 else -1
                if res_id == -1:
                    continue

                ss_char = line[16] if line_length > 16 and line[16] in map_ss_8 else ' '
                ss_vector = map_ss_8.get(ss_char, map_ss_8[' '])

                ASA = safe_float(line[34:38]) if line_length > 38 else 0.0
                TCO = safe_float(line[74:81]) if line_length > 81 else 0.0
                KAPPA = safe_float(line[81:86]) if line_length > 86 else 360.0
                ALPHA = safe_float(line[86:91]) if line_length > 91 else 360.0
                PHI = safe_float(line[91:96]) if line_length > 96 else 360.0
                PSI = safe_float(line[96:101]) if line_length > 101 else 360.0
                X_CA = safe_float(line[101:107]) if line_length > 107 else 0.0
                Y_CA = safe_float(line[107:113]) if line_length > 113 else 0.0
                Z_CA = safe_float(line[113:119]) if line_length > 119 else 0.0

                # 标准化特征
                feature = np.zeros([14])
                feature[:8] = ss_vector
                feature[8] = min(ASA / maxASA.get(aa, 1), 1)
                feature[9] = TCO
                feature[10] = min(KAPPA / 180, 1)
                feature[11] = min((ALPHA + 180) / 360, 1)
                feature[12] = min((PHI + 180) / 360, 1)
                feature[13] = min((PSI + 180) / 360, 1)

                # 存储特征
                seq_feature[res_id] = feature.reshape((1, -1))

            except ValueError:
                print(f"DSSP 解析错误：文件 {file} 第 {i + 1} 行数据异常，已跳过")
                continue

        if not seq_feature:
            print(f"{file} 没有解析出任何特征，跳过存储！")
            continue

        dssp_dict[seqid] = seq_feature

    # 保存特征
    output_file = os.path.join(feature_dir, f"{ligand}_SS.pkl")
...
if __name__ == '__main__':
    ...
    trainingset_dict = {'PPIS': 'train_186_164.txt'}

    testset_dict = {'PPIS': 'test_71.txt'}

    # Dataset_dir = os.path.abspath('..') + '/Datasets' + '/' + ligand
    #Dataset_dir = '/home/mijia/egpdi_evolla/data'
    #AF3版data
    Dataset_dir = '/home/mijia/egpdi_evolla/af3_data'
    #pdb文件夹
    #PDB_chain_dir = Dataset_dir + '/pdb_dir'
    #AF3版pdb
    PDB_chain_dir = Dataset_dir + '/af3_pdb_dir'
    #训练集文件：
    ...
    Dataset_dir_train_186_164_test_71 = Dataset_dir + '/dataset_dir_train_186_164_test_71'
    #数据集id合集
    ...
    # psepos:核心原子
    cal_Psepos(seqlist, PDB_DF_dir, Dataset_dir_train_186_164_test_71, psepos, ligand, seqanno)
    ...
    cal_PSSM(ligand, seqlist, Dataset_dir + '/PSSM', Dataset_dir_train_186_164_test_71)
    #cal_HMM(ligand, seqlist, Dataset_dir_train_186_164 + '/HMM', Dataset_dir)
    cal_DSSP(ligand, seqlist, Dataset_dir + '/SS', Dataset_dir_train_186_164_test_71)
    #处理原子特征
    PDBResidueFeature(seqlist, PDB_DF_dir, Dataset_dir_train_186_164_test_71, ligand, feature_list, feature_combine, atomfea)

    root_dir = Dataset_dir_train_186_164_test_71 + '/' + ligand + '_{}_dist{}_{}'.format(psepos, dist, feature_combine)
    raw_dir = root_dir + '/raw'
    if os.path.exists(raw_dir):
        shutil.rmtree(root_dir)
    os.makedirs(raw_dir)
    print('4.Calculate the neighborhood of residues. Save to {}.'.format(root_dir))
    Create_NeighResidue3DPoint(psepos, dist, Dataset_dir_train_186_164_test_71, raw_dir, seqanno, feature_combine, train_list, valid_list,
                               test_list)
