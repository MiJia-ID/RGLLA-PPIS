...
class U_GCN_EGNN(nn.Module):
    ...
    def forward(self, G, H, X, adj, Efeats): # G:graph_batch, h:node_features_batch, x:coors_batch, adj:adj_batch, efeats: efeat_batch
        ...
        for i in range(self.l_n):
            # if self.ks[i] != 1:
                # print("-----------第" + str(i + 1) + "次dropout---------------")
                # print(f"pool之前的efeats形状{Efeats.shape}")
                # print(f"pool之前的coor形状{x.shape},pool之后的idx是{idx.shape}")
                # print(f"pool之前的G的点{g.number_of_nodes()}，边{g.number_of_edges()}")
                # print(f"pool之前的h形状{H.shape}")
                if i==0:
                    h=H
                    x=X
                    g=G
                ...
                if i==2:
                    ...
                    #print("-----------第2次dropout---------------")
                    fea = self.linear(fea)
                    #print(f"pool之前的fea形状{fea.shape}")
                ...
                for j, con in enumerate(self.convs):
                    ...
                    # print("-----------第"+str(j+1)+"次EGNN---------------")
                    # print(f"pool之后的efeats形状{efeats.shape}")
                    # print(f"pool之后的coor形状{x.shape},pool之后的idx是{idx.shape}")
                    # print(f"pool之后的G的点{g.number_of_nodes()}，边{g.number_of_edges()}")
                    # print(f"pool之后的h形状{h.shape}")
                    h = self.act_fn(
                        # self, input, coord_feat, h0, lamda, alpha, l, adj_matrix=None, graph=None, efeats=None
                ...
                # print("-----------第" + str(i + 1) + "次EGNN已Unpool---------------")
            # else:
            #     # print("第" + str(i + 1) + "次dropout：" + str(self.ks[i]))
            #     h = self.rgn_gcn2(H, adj)
            #     # print(f"经过RGN_GCN之后的节点形状{h.shape}")
...
class RGN_GCN(nn.Module):
    ...
    def forward(self, h, adj):
        _layers = []
        h = F.dropout(h, self.dropout, training=self.training)
        # print(f"经过RGN_GCN之前的节点形状{h.shape}")
        layer_inner = self.act_fn(self.fcs[0](h))
        _layers.append(layer_inner)
