...
from sklearn.metrics import matthews_corrcoef,  precision_score, recall_score, f1_score,roc_auc_score, average_precision_score,confusion_matrix,accuracy_score
...
parser.add_argument("--edgefeats_path", type=str, default='/home/mijia/egpdi_evolla/af3_data/dataset_dir_train_186_164_test_71/test_71/edge_features/EdgeFeats_predicted_SC_17_181.pkl')
...
with open(test_path,'r') as f:
    test_text = f.readlines()
    for i in range(0, len(test_text), 3):
        seq = test_text[i+1].strip()
        label = test_text[i+2].strip()
        sequences.append(seq)
        labels.append(label)

sequences = sequences[:71]
labels = y_test
features = X_test
...
def test_129(Model_Path):
    model = MainModel(dr, lr, nlayers, lamda, alpha, atten_time, nfeats=41 + 20 + 1024 + 5120).to(device)
...
aupr = max(test_results_1['aupr'],test_results_2['aupr'],test_results_3['aupr'],test_results_4['aupr'],test_results_5['aupr'])
acc = max(test_results_1['acc'],test_results_2['acc'],test_results_3['acc'],test_results_4['acc'],test_results_5['acc'])
thred = (test_results_1['thred']+test_results_2['thred']+test_results_3['thred']+test_results_4['thred']+test_results_5['thred'])/5

print("Test_71 performance on our method")
...
print("final aupr: {:.3f} ".format(aupr))
print("final acc: {:.3f} ".format(acc))
#nohup python test_129_final.py 2>&1 &
